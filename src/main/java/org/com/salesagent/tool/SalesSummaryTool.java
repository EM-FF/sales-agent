...
public class SalesSummaryTool {
    ...
    public String getTopReps(
        ...
        try {
            ...
            sb.append(String.format("销售员业绩排名（%s 至 %s%s）:\n\n",
                    startDate, endDate, regionName != null && !regionName.isBlank() ? "," + regionName : ""));

            int i = 1;
            for (RepSalesDTO rep : reps) {
                sb.append(String.format("第 %d 名：%s（%s） 销售额：￥%,.0f\n",
                        i++, rep.repName(), rep.regionName(), rep.totalAmount().doubleValue()));
            }

            return sb.toString();
        }catch (DateTimeParseException e){
            return "日期格式错误，请使用yyyy-MM-dd 格式";
        }catch(Exception e){
            log.error(e.getMessage());
            return "查询排名数据出现问题，请稍后重试";
        }
    }
    ...
    public String getRegionRanking(
        ...
        try {
            ...
            int i = 1;
            for (RegionSalesDTO region : regions) {
                double radio = grandTotal.compareTo(BigDecimal.ZERO) > 0 ?
                        region.totalAmount().doubleValue() / grandTotal.doubleValue() * 100 : 0;
                sb.append(String.format("第 %d 名：%s  销售额：￥%,.0f 占比：%.1f%%\n",
                        i++, region.regionName(), region.totalAmount(), radio));
            }
            return sb.toString();
        }catch (DateTimeParseException e){
    ...
    public String getTopProducts(
        ...
        try {
            ...
            // 如果查最差的，倒序
            if (isWorst) {
                // 截取products最后n个数据：n < products.size() 左区间变成products.size() - n；n > products.size() 直接截取整个products
                products = products.subList(Math.max(0, products.size() - n), products.size());
                products = new ArrayList<>(products);
                Collections.reverse(products);
            } else {
                products = products.subList(0, Math.min(products.size(), n));
            }

            StringBuilder sb = new StringBuilder();
            sb.append(String.format("产品销售排名：（%s至%s）：\n",
                    isWorst ? "(最差)" : "(最好)", startDate, endDate));

            int i = 1;
            for (ProductSalesDTO product : products) {
                sb.append(String.format("第 %d 名：%s [%s] 品类：%s 销售额：￥%,.0f 数量 %d 件\n",
                        i++, product.productName(), product.skuCode(), product.category(),
    ...
    public String getSalesSummary(
        ...
        log.info("工具调用-getSalesSummary: start={}, end={}, regionName={}", startDate, endDate, regionName);
        ...
        try {
            ...
            return "日期格式错误，请按照 yyyy-MM-dd 格式";
        } catch (Exception e){
            log.error(e.getMessage());
            return "查询汇总数据时出现错误，请稍后重试";
        }
    }
